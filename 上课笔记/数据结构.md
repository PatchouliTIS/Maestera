...
|归并趟数|比较次数|总时间复杂度|稳定性|顺序表|链表|
|:--:|:--:|:--:|:--:|:--:|:--:|
|o(log<sub>2</sub>n)|每一趟归并比较n~2n-1次，与初态有关。(o(n))|归并$\times$比较=**o(nlog<sub>2</sub>)**|√|√|×（基于分治法）|
...
**目的**：如何在**不增大内部归并时间**且**不增大外部存取时间**的前提下提高归并路数k——**败者树**
...
**核心**：将工作区WA填满，找到比上一个记录最小值w<sub>min</sub>大的记录集中最小的那一个记录，放到FO中，直到无法找到这样的记录值。
...
**目的**：在I/O次数最少的情况下**组织非等长归并段的合理归并顺序**
